...
(
    id                  UUID                      NOT NULL,
    aggregate_type      VARCHAR(50)               NOT NULL, -- type is for different message types, for now only `delivery` is supported
...
CREATE INDEX outbox_message_process_status_idx
    ON public.outbox_message (type, status, created_at) WHERE status = 'NEW' OR status = 'WAITING_RETRY';

CREATE INDEX outbox_message_cleanup_idx
    ON public.outbox_message (processed_at) WHERE status = 'PROCESSED';

CREATE INDEX outbox_message_failed_idx
    ON public.outbox_message (id) WHERE status = 'FAILED';